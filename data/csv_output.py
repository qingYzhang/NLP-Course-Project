...
def write_to_csv(data, file_name):
    ...
    with open(file_name, 'w', newline='', encoding='utf-8') as file:
        ...
        for index, lines in enumerate(data):
            last_word = lines[0].split()[-1]

            try:
                writer.writerow({'Line': last_word, 'Groupnum': ','.join(map(str, lines))})
            except UnicodeEncodeError:
                cleaned_lines = [','.join(map(str, line.split())) if isinstance(line, str) else line for line in lines]
                writer.writerow({'Line': ''.join(c if ord(c) < 128 else '' for c in last_word), 'Groupnum': ','.join(cleaned_lines)})

def main():
    annotator = Annotator()

    lyrics = pd.read_csv("./lyrics_rhyming_pairs.csv")
    print(lyrics)
    # print(lyrics["next_line"])
    grouped_lines = []
    lines = lyrics["next_line"]
    print(lines)
    for line in lines:
        ...
        # print(row["Content"])
        print(line)
        # Create groups of rhyming lines
        rhyme_groups = []
        # print(len(lines))
        for i in range(len(lines)-1):
            ...
            for j in range(1, 3):
                try:
                    ...
                    if annotator.is_rhyme(line[-1], next_line[-1]):
                        rhyme_groups.append([lines[i], lines[i+j]])
                except Exception:
                    continue

        grouped_lines.extend(rhyme_groups)

    # Print and save the grouped lines
    print(grouped_lines)
    for group in grouped_lines:
        print(group)
    write_to_csv(grouped_lines, "grouped_lyrics.csv")
